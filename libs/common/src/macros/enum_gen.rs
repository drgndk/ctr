  ...
  (
    ...
    impl $enum_name {
      ...
      pub fn to_string(self) -> String {
        match self {
          $(
            Self::$variant_name $({ $($field_name),* })? $({ $($tuple_name),* })? => stringify!($variant_name),
          )*
        }.to_string()
      }
      ...
      pub fn try_from(string: impl Into<String>) -> Option<Self> {
        ...
        match string.to_lowercase() {
          $(
            s if s.eq(&stringify!($variant_name).to_lowercase()) => Some(Self::$variant_name $({ $($field_name: $field_default),* })? $({ $($tuple_name: Default::default()),* })?),
          )*
          _ => None,
        }
      }
      ...
      $(
        paste::paste! {
          #[allow(dead_code)]
          pub fn [<new_ $variant_name:lower>]($($($field_name: $field_type),* )? $($($tuple_name: $tuple_type),* )?) -> Self {
            Self::$variant_name $({ $($field_name),* })? $({ $($tuple_name),* })?
          }
        }
      )*
