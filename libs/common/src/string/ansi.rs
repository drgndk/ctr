...
struct_gen! {
  ...
  mod vec_functions {
    ...
    pub fn insert(self: &mut Self, index: usize, effect: Effect) {
      // If the NO_COLOR environment variable is set, don't add any effects.
      if *NO_COLOR {
        return;
      }
      ...
      // If the current effect is the same as the last effect, don't add it.
      if let Some(last_effect) = self.effects.get(index) {
        if last_effect == &effect {
          return;
        }
      }
    ...
    pub fn pop(self: &mut Self) -> Option<Effect> {
      self.effects.pop()
    }
    ...
    pub fn truncate(self: &mut Self, index: usize) {
      self.effects.truncate(index);
    }
    ...
    pub fn last(self: &Self) -> Option<&Effect> {
      self.effects.last()
    }

    pub fn iter(self: &Self) -> std::slice::Iter<Effect> {
      self.effects.iter()
    }

    pub fn retain(self: &mut Self, f: impl FnMut(&Effect) -> bool) {
      self.effects.retain(f);
    }

    pub fn to_ansi(self: &Self) -> String {
      let value = if self.effects.is_empty() {
        String::new()
      } else {
        if self.effects.contains(&Effect::Reset) {
          return Effect::Reset.to_ansi();
        }
        self.effects.iter().map(|e| e.get_ansi_value().to_string()).collect::<Vec<String>>().join(";")
      };
